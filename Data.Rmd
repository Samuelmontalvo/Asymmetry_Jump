...
Demographics <- df_demo %>%
  ...
  mutate(
    dob = lubridate::dmy(dob)  # convert DOB to Date
  )

## Prepare jump data and clean numeric columns (including type and tags)
df_jump <- load_jump_data()
...
df <- df_jump %>%
  ...
  select(
    name,
    date,
    type,                        # include type for later tables
    tags,                        # include tags for later tables
    left_avg_propulsive_force,
    right_avg_propulsive_force
  ) %>%
  mutate(
    # recode "N/A" to NA, then parse numbers
...
## Verify that df2 now contains type and tags along with numeric forces
glimpse(df2)
```
