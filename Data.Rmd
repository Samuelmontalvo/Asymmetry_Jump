...
# 1) Load raw data and clean column names
df_jump <- load_jump_data()
df <- df_jump %>%
  janitor::clean_names()

# 2) Select only the necessary columns (including 'name')
df <- df %>%
  select(
    name,
    date,
...
# 3) Date transformations and derived variables
df <- df %>%
  mutate(
...
# 4) Perform inner join with demographics
...
df <- df_jump %>%
  ...
  mutate(
    # Recode literal "N/A" to NA
    left_avg_propulsive_force  = na_if(left_avg_propulsive_force,  "N/A"),
    right_avg_propulsive_force = na_if(right_avg_propulsive_force, "N/A"),
    # parse_number strips non-numeric chars and converts to numeric
    left_avg_propulsive_force  = readr::parse_number(left_avg_propulsive_force),
    right_avg_propulsive_force = readr::parse_number(right_avg_propulsive_force),
    # Date transforms
    date       = lubridate::dmy(date),
    month      = factor(format(date, "%Y-%m")),
    session_id = interaction(name, date),
  ...
  filter(!is.na(date)) %>%
  # Drop rows missing either force measurement
  tidyr::drop_na(left_avg_propulsive_force, right_avg_propulsive_force)

## Join with demographics
df2 <- inner_join(df, Demographics, by = "name")

## Check the structure and ensure no NAs remain in force columns
glimpse(df2)
```
