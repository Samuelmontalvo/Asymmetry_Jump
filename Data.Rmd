...
df_demo <- janitor::clean_names(df_demo)

Demographics <- df_demo %>%
  select(
    athlete,                 # original participant name
    date_of_birth_dd_mm_yy,  # original DOB
    sex,
    height
  ) %>%
  rename(
    name = athlete,
    dob  = date_of_birth_dd_mm_yy
  ) %>%
  mutate(
    dob = lubridate::dmy(dob)  # convert DOB string to Date
  )

# Load and clean jump data (including propulsive forces)
...
df <- df_jump %>%
  ...
  select(
    name,
    date,
    type,                      # jump type for descriptive tables
    tags,                      # tags for descriptive tables
    system_weight,             # system weight in N
    jump_height,               # jump height in m
    rsi,                       # reactive strength index
    m_rsi,                     # modified RSI
    peak_propulsive_force,     # peak force in propulsive phase
    left_avg_propulsive_force, # average left propulsive force
    right_avg_propulsive_force # average right propulsive force
  ) %>%
  mutate(
    ...
    # Derived variables
    mass           = system_weight / 9.81,  # convert N to kg
    jump_height_cm = jump_height * 100      # convert m to cm
  ) %>%
  filter(!is.na(date)) %>%
  tidyr::drop_na(
...
## Prepare jump data and clean numeric columns
df_jump <- load_jump_data()
...
df <- df_jump %>%
  ...
  select(
    ...
    type,
    tags,
    system_weight,     # from "System Weight"
    jump_height,       # from "Jump Height"
    rsi,               # from "RSI"
    m_rsi,             # from "mRSI"
    left_avg_propulsive_force,
    right_avg_propulsive_force
  ...
  filter(!is.na(date)) %>%
  tidyr::drop_na(left_avg_propulsive_force, right_avg_propulsive_force)
