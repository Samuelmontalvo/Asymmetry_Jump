...
df_demo <- load_demographics() %>% janitor::clean_names()
Demographics <- df_demo %>%
  select(athlete, date_of_birth_dd_mm_yy, sex, height) %>%
  rename(name = athlete, dob = date_of_birth_dd_mm_yy) %>%
  mutate(dob = lubridate::dmy(dob))

## Load and clean jump data (including countermovement_depth)
df_jump <- load_jump_data()
...
df <- df_jump %>%
  ...
  select(
    ...
    left_force_at_peak_propulsive_force,
    right_force_at_peak_propulsive_force,
    peak_relative_braking_force,
    peak_relative_propulsive_force,
    flight_time
  ) %>%
  mutate(
    # Parse numeric strings
    peak_propulsive_force     = readr::parse_number(as.character(peak_propulsive_force)),
    left_avg_propulsive_force  = readr::parse_number(na_if(left_avg_propulsive_force,  "N/A")),
    right_avg_propulsive_force = readr::parse_number(na_if(right_avg_propulsive_force, "N/A")),
    countermovement_depth     = readr::parse_number(as.character(countermovement_depth)),
    left_force_at_peak_propulsive_force = readr::parse_number(as.character(left_force_at_peak_propulsive_force)),
    right_force_at_peak_propulsive_force = readr::parse_number(as.character(right_force_at_peak_propulsive_force)),
    peak_relative_braking_force = readr::parse_number(as.character(peak_relative_braking_force)),
    peak_relative_propulsive_force = readr::parse_number(as.character(peak_relative_propulsive_force)),
    flight_time               = readr::parse_number(as.character(flight_time)),

    # Date variables
    date     = lubridate::dmy(date),
    month    = factor(format(date, "%Y-%m")),
    session_id = interaction(name, date),
    week     = lubridate::isoweek(date),

    # Derived variables
    mass         = system_weight / 9.81,
...
df2 <- inner_join(df, Demographics, by = "name")
...
# 2) Calculate age in years from dob to test date
df2 <- df2 %>%
  mutate(
    Age = interval(dob, date) / years(1)
  )

# 3) Optionally inspect the new column
df2 %>%
  select(name, date, dob, Age) %>%
  glimpse()
```

```{r}
# (Re)define df3 a partir de df2, si lo deseas
df3 <- df2

# Ahora sí table1 funciona porque df3 tiene type y tags
library(table1)
label(df3$type) <- "Jump Type"
label(df3$tags) <- "Tags"
...
# Plot 1: Total Peak Propulsive Force (using df2 directly)
p2 <- ggplot(df2, aes(x = date, y = peak_propulsive_force, group = name, color = name)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
    title = "Total Peak Propulsive Force",
    x     = "Date",
    y     = "N"
  ) +
  theme(legend.position = "none")

# Plot 2: Left Leg Avg Propulsive Force
p3 <- ggplot(df2, aes(x = date, y = left_avg_propulsive_force, group = name, color = name)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
    title = "Left Leg Avg Propulsive Force",
    x     = "Date",
    y     = "N"
  ) +
  theme(legend.position = "none")

# Plot 3: Right Leg Avg Propulsive Force
p4 <- ggplot(df2, aes(x = date, y = right_avg_propulsive_force, group = name, color = name)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
    title = "Right Leg Avg Propulsive Force",
    x     = "Date",
    y     = "N"
  ) +
  theme(legend.position = "none")
...
```
```{r}
## Compute asymmetry indices in df2
df2 <- df2 %>%
  mutate(
    A      = pmax(right_avg_propulsive_force, left_avg_propulsive_force),
    B      = pmin(right_avg_propulsive_force, left_avg_propulsive_force),
    Index1 = abs(A - B) / A * 100,
    Index2 = (A - B) / (A + B) * 100,
    Index3 = (45 - atan(B / A) * (180 / pi)) * 100 / 90,
    Index4 = ifelse(A > 0 & B > 0, log(A / B) * 100, NA)
  )
...
## Covariates: Mass
model1m <- lmer(
  jump_height_cm ~ week * Index1 + mass +
    (1 | name) + (1 + countermovement_depth | session_id),
  data = df2
)

## Without mass
model1nom <- lmer(
  jump_height_cm ~ week * Index1 +
    (1 | name) + (1 + countermovement_depth | session_id),
  data = df2
)
...
## Covariates: Age
model1a <- lmer(
  jump_height_cm ~ week * Index1 + Age +
    (1 | name) + (1 + countermovement_depth | session_id),
  data = df2
)
...
feature_matrix <- df %>% select(-jump_height_cm)
label_vector <- df$jump_height_cm
...
```{r xgboost_prediction, echo=FALSE, eval=FALSE}
# ## XGBoost prediction code temporarily disabled pending debug
# # 1) Build test grid of Index3 values
# # 2) Manually one-hot encode sex (sexFemale, sexMale)
# # 3) Fill missing feature columns with zeros
# # 4) Reorder to match model feature order
# # 5) Convert to matrix and call predict(xgb_cv_model, …)
# # These steps currently error with:
# # “Feature names stored in object and newdata are different!”
```

```{r plot_asymmetry_pred, echo=FALSE, eval=FALSE}
# ## Plotting code disabled pending debug of XGBoost predictions
# library(ggplot2)
#
...
#   geom_point(size = 3) +
#   geom_ribbon(aes(
#     ymin = Predicted_Jump_Height - 1.96 * 1.556,
#     ymax = Predicted_Jump_Height + 1.96 * 1.556,
#     fill = as.factor(sex)
#   ), alpha = 0.2) +
#   labs(
