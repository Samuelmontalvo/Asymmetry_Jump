...
jumps_df <- janitor::clean_names(jumps_raw_df)
demographics_df <- janitor::clean_names(demographics_raw_df)
...
ggsave(filename = "figure_asymmetry_timeseries_300dpi.tiff", # Choose filename and format (.tiff, .png, .pdf, .jpeg)
...
# Save the plot for publication
ggsave(filename = "figure_asymmetry_vs_jumpheight_300dpi.tiff", # Choose filename and format
...
tryCatch({
  ...
  if (length(interaction_term_row) == 1) { # Check if term was found
      ...
      if (!is.na(p_value_interaction) && p_value_interaction < 0.05) { # Using p < 0.05 threshold
         ...
         ggsave(filename = "figure_lmm_interaction_avgf_300dpi.tiff",
...
tryCatch({
  ...
  if (length(interaction_term_row_peakf) == 1) {
      ...
      if (!is.na(p_value_interaction_peakf) && p_value_interaction_peakf < 0.05) {
         ...
         ggsave(filename = "figure_lmm_interaction_peakf_300dpi.tiff",
...
tryCatch({
  ...
  if (length(interaction_term_row_imp) == 1) {
      ...
      if (!is.na(p_value_interaction_imp) && p_value_interaction_imp < 0.05) {
         ...
         ggsave(filename = "figure_lmm_interaction_impulse_300dpi.tiff",
...
tryCatch({
  ...
  ggsave(filename = "figure_xgb_importance_300dpi.tiff",
...
ggsave(filename = "figure_xgb_predictions_thresholds_300dpi.tiff",
